...
const CurrentQue = () => {
  ...
  // ---------------pagination--------------------------
  const [currentOwnerCity, setCurrentOwnerCity] = useState();

  useEffect(() => {
    if (typeof window !== "undefined") {
      setCurrentOwnerCity(
        JSON.parse(localStorage.getItem("user")).police[0].city
      );
    }
    console.log(currentOwnerCity, typeof currentOwnerCity);
  }, []);
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    let params = {
      ...
      checked: false,
      qued: true,
      currentOwnerCity: JSON.parse(localStorage.getItem("user")).police[0].city,
    };
        ...
        if (data.status && data.status == "success") {
          if (data.results == 0) {
            setAlert({
              ...alert,
              loading: true,
              message: "There are no cases at this area till now..",
              error: false,
              success: true,
            });
    ...
    <>
      <Layout>
        ...
        <div className="mt-2 mb-2 p-2 mr-10 border-2 border-gray-200 rounded-xl grid grid-cols-4">
          ...
          {currentOwnerCity && <SearchBar city={currentOwnerCity} />}
          {/* )} */}
          {/* {
                if (typeof window !== "undefined") {
